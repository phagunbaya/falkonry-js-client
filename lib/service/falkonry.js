...
FalkonryService.prototype.createEventbuffer = function(eventbuffer, options, done) {
  ...
  if(options.thingIdentifier) {
    formData['thingIdentifier'] = options.thingIdentifier;
  }
  else{
    if(options.signalsTagField) {
      formData['signalsTagField'] = options.signalsTagField;
    }
    if(options.signalsDelimiter) {
      formData['signalsDelimiter'] = options.signalsDelimiter;
    }
    if(options.signalsLocation){
      formData['signalsLocation'] = options.signalsLocation;
    }
    if(options.valueColumn) {
      formData['valueColumn'] = options.valueColumn;
    }
  }
  return _this.http.post('/Eventbuffer', formData, function(error, response, code){
    if(error)
      return done(error, null, code);
    else {
